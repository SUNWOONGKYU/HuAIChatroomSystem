...
// 했다. 소대장이 아닌 봇이 이기면 그 봇 입장에선 "나한테 한 말이 아니다" 로 관찰로
// 떨어지고, 소대장의 (진짜 정답인) 판정은 뒤늦게 도착해도 이미 그 슬롯이 차 있어
// 409 로 버려진다 — 방장이 소대장을 불러도 무응답.
//
...
const PLATOON = "leader_chatroom_bot";
const CLAUDE = "claude_chatroom_bot";
const CODEX = "codex_chatroom_bot";
const AUDITOR = "auditor_chatroom_bot";

const ROLE_BY_USERNAME: Record<string, "platoon_leader" | "claude_leader" | "codex_leader" | "auditor"> = {
  [PLATOON]: "platoon_leader",
  [CLAUDE]: "claude_leader",
  [CODEX]: "codex_leader",
  [AUDITOR]: "auditor"
};

for (const [winnerLabel, winnerUsername] of Object.entries({ auditor: AUDITOR, codex: CODEX, claude: CLAUDE, platoon: PLATOON })) {
  test(`${winnerLabel} 봇이 기록 경쟁에서 먼저 이겨도 @leader_chatroom_bot 멘션은 결국 소대장이 처리한다`, async () => {
    const store = raceAwareStore();
    const config = fourBotConfig();
    const text = "@" + PLATOON + " 뭐 하고 있어";
    const messageId = 7001;
    let nextUpdateId = 30000;

    // 1 사이클: winner 봇(과 나머지, 소대장 제외)이 먼저 자기 사본을 받는다 —
    // 실제로는 봇마다 독립적인 getUpdates 배달 타이밍과, 순차 폴링(봇 하나당
    // 최대 25초 대기)이 겹쳐서 흔히 벌어지는 상황이다. winner 가 platoon 자신이면
    // 경쟁이랄 것도 없이 정상 케이스(양성 대조)가 된다.
    const firstCycleUsernames = Object.values({ auditor: AUDITOR, codex: CODEX, claude: CLAUDE, platoon: PLATOON })
      .filter((username) => username === winnerUsername || username !== PLATOON);
    // winner 가 platoon 이 아니면 platoon 은 이번 사이클에서 뺀다(뒤늦게 도착 흉내).
    const firstCycleBots = firstCycleUsernames
      .filter((username, index, all) => all.indexOf(username) === index)
      .filter((username) => winnerUsername === PLATOON || username !== PLATOON)
    ...
    if (winnerUsername !== PLATOON) {
      // 이 시점에 소대장 아닌 봇들은 전부 "나한테 온 말 아님" 이어야 한다 —
      // 아무도 소대장 대신 이 멘션을 처리하면 안 된다.
      const misrouted = store.queued.filter((m) => m.input.kind !== "observation");
      assert.equal(misrouted.length, 0, "소대장이 아닌 봇이 이 멘션을 대신 처리하면 안 된다");

      // 2 사이클: 소대장의 자기 사본이 뒤늦게 도착.
      await runTelegramPollingCycle({
        bots: [bot(PLATOON)],
        config,
        ports: store.ports,
        offsets: new TelegramPollingOffsets(),
        deps: { fetchImpl: perBotFetch([[PLATOON, [message(nextUpdateId++, CHAT, messageId, text)]]]) }
      });
    }

    const platoonHandled = store.queued.filter(
      (m) => m.input.envelope.telegramBotRole === "platoon_leader" && (m.input.kind === "message" || m.input.kind === "command")
    );
    assert.equal(platoonHandled.length, 1, `${winnerLabel} 가 먼저 기록을 이겨도 소대장이 결국 이 멘션을 처리해야 한다`);
  });
}
...
test("이름만 부르는 지시(예: '소대장 정리해줘')도 다른 봇이 먼저 기록을 이겨도 결국 소대장이 처리한다", async () => {
  const store = raceAwareStore();
  const config = fourBotConfig();
  const text = "소대장 이거 정리해줘";
  ...
  await runTelegramPollingCycle({
    ...
    ports: store.ports,
    offsets: new TelegramPollingOffsets(),
    deps: { fetchImpl: perBotFetch([[PLATOON, [message(50001, CHAT, messageId, text)]]]) }
  });

  const platoonHandled = store.queued.filter((m) => m.input.envelope.telegramBotRole === "platoon_leader" && m.input.kind === "message");
  assert.equal(platoonHandled.length, 1);
});

test("무시 판정 사유가 관측 가능하게 남는다(onIgnored 콜백)", async () => {
  const store = raceAwareStore();
  const config = fourBotConfig();
  const text = "@" + PLATOON + " 해줘";
...
function fourBotConfig(): BotServiceConfig {
  return {
    ...
    botsByUsername: new Map(
      [PLATOON, CLAUDE, CODEX, AUDITOR].map((username) => [
        username,

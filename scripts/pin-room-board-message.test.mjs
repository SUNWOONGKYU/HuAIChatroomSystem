...
test("우리가 올린 작업 현황판이 이미 고정돼 있으면 다시 만들지 않는다", () => {
  // 이걸 놓치면 실행할 때마다 방에 작업 현황판 메시지가 하나씩 더 쌓인다.
...
test("우리 봇이 올렸어도 작업 현황판이 아닌 메시지면 우리 것으로 보지 않는다", () => {
...
test("고정 메시지 링크에 그 방의 roomId 가 실린다", () => {
  // 방마다 다른 링크여야 한다. 하나라도 섞이면 다른 방 작업 현황판이 열린다.

...
// 쌓여 협업 운영센터 첫 화면을 122건으로 채웠다. Telegram 은 메시지가 흘러가서 이게 안
// 보였다 — 협업 운영센터가 목록으로 쌓아 보여주면서 처음 드러났다.
//
// 왜 삭제가 아니라 취소 기록인가: huai_events 는 append-only 다(huai_events_append_only).
// 제안이 있었다는 사실 자체는 지우면 안 되고, "결정됐다"를 승인 원장에 남기는 것이
// 이 시스템이 이미 쓰는 방식이다.
//
// stage 를 'cancellation' 이 아니라 'task_approval' 로 넣는 이유: 협업 운영센터가 미결을
...
export function selectStaleProposals(events, decidedEntityRefs) {
  ...
  for (const row of events) {
    ...
    // 'leader_planning_requested'). 협업 운영센터가 애초에 안 보여주므로 재고가 아니다.

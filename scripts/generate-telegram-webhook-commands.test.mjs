...
test("generates four role webhook commands without embedding token values", () => {
  ...
  assert.equal(commands[0].includes("https://ops.example.com/telegram/webhook/leader_live_bot"), true);
  assert.equal(commands[0].includes("$BOT_SERVICE_LEADER_BOT_TOKEN"), true);

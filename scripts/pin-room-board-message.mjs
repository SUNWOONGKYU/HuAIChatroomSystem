...
//
// 왜 필요한가: 협업 운영센터 버튼은 /tasks 응답에 붙는데, 그 메시지는 대화가 쌓이면 위로
// 밀려 올라가 찾을 수 없다. 방장이 협업 운영센터를 열 때마다 /tasks 를 다시 쳐야 한다.
...
// 협업 운영센터 전용 메시지를 따로 만든다 — 본문은 안 낡고, 최신 상태는 버튼을 눌러서 본다.
...
// "협업 운영센터") 이미 고정해 둔 우리 메시지를 남의 것으로 오인해서, 갱신도 못 하고

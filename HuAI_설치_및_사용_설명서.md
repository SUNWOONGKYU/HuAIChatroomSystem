...
사람 말투로 지시하면, AI(Claude·Codex)가 실제로 파일을 만들고 고치고 결과를 방에 보고합니다.
...
**[전문 보기]** 버튼이 붙습니다 — 눌러 협업 운영센터에서 전체를 봅니다.

협업 운영센터(작업 목록·진행률·전문·완료 승인)는 텔레그램 봇 메뉴의 `/center` 또는 작업 메시지의
협업 운영센터 링크로 엽니다. 포럼 주제에서 `/center`를 선택하면 현재 방·주제가 반영됩니다.

## 완료 승인하기

작업이 끝나면 **협업 운영센터에서** 완료 승인을 합니다(방 안에는 완료 승인 버튼이 따로 없습니다 —
"완료 승인은 협업 운영센터에서"라고 안내가 뜹니다).

**퀴즈가 뜰 수 있습니다**: 배포·삭제·권한/인증·환경변수·시크릿·중요 설정·DB 스키마 변경처럼
고위험 작업은, 완료 승인을 누르기 전에
"뭐가 바뀌었는지 이해했는지" 확인하는 객관식 3문항이 먼저 나옵니다(인지부채 방지 장치 —
AI에게 맡기기만 하고 아무도 내용을 모르는 상태를 막기 위해서입니다). 조회·분석·설명·검토와
단순 저위험 파일 변경은 퀴즈 없이 승인할 수 있습니다. 통과해야 진짜
협업 운영센터에 완료 승인 버튼이 나옵니다.

synthetic LeaderBot 계획 테스트는 실제 운영 데이터 없이 실행합니다.
...
테스트 파일은 `apps/bot-service/test/synthetic-leader-planning-webhook.test.ts`이며 가상 방의 방장 5001·참여자 9001 4턴, 가짜 webhook/fetch만 사용합니다. 실제 Telegram/Supabase에는 쓰지 않습니다. 게임 로컬 테스트는 시작·키보드·드래그·충돌·재시작·반응형을 통과했지만 30초 생존은 테스트 훅, 공개 URL 브라우저 여정은 환경 제약으로 미검증입니다.
...
GitHub에는 소스·문서·`.env.operation.example`만 올립니다. `.env.operation.local`, Telegram
토큰, Supabase service role 키, webhook secret, CLI 인증 파일은 절대 커밋하지 않습니다.
...
운영 PC에 반영할 때는 `git pull` 후 `npm install` → `npm run build` → 환경 검증 → 서비스
재시작 → `node --env-file=.env.operation.local scripts/set-telegram-bot-commands.mjs --apply`
순서로 진행합니다.

...
// "Uncaught" 로 실패했고, verify-operation-ready.mjs 의 내장 재시도(retryCount=1)가 그걸
// 삼켜서 겉으로는 통과처럼 보였다(5차·6차 감사 연속 지적). 재시도가 flakiness 를 가리면
// 나중에 진짜 결함이 났을 때도 같이 가려진다.
...
await waitUntil(`Boolean(window.__breakout) && location.href === ${JSON.stringify(gameUrl)}`, "게임 페이지가 열리고 준비되기를");
